...
import { CreateTaskPayload, TaskCache, UpdateTaskPayload } from "@/types";
...
type TaskStore = {
  ...
  updateTasks: (colIds: string[], payload: UpdateTaskPayload[]) => Promise<TaskCache[] | null>;
  deleteTasks: (colIds: string[]) => Promise<void>;
  fetchTasksByColumns: (
    colIds: string[],
    limitTasks: number,
  ) => Promise<TaskCache[]>;
...
export const useTaskStore = create<TaskStore>((set) => ({
  ...
  createTasks: async (payload) => {
    ...
    try {
      ...
      const tasksMap = toRecord(createdTasks, "id");
      set((state) => ({
        tasks: {
          ...state.tasks,
          ...tasksMap,
  ...
  updateTasks: async (colIds, payload) => {
    if (colIds.length === 0 || payload.length === 0) {
        toast.error("No tasks to update");
      return null;
    }
    set({ status: "updating" });
    try {
      const response = await taskServices.updateTasks(colIds, payload);
      ...
      const tasksMap = toRecord(updatedTasks, "id");
      set((state) => ({
        tasks: {
          ...state.tasks,
          ...tasksMap,
  ...
  deleteTasks: async (colIds) => {
    ...
    set({ status: "deleting" });
    try {
      const response = await taskServices.deleteTasks(colIds);
      ...
      set((state) => {
        ...
        colIds.forEach((id) => delete newTasks[id]);
        return {
          tasks: newTasks,
          status: "none",
        };
  ...
  fetchTasksByColumns: async (colIds, limitTasks) => {
    ...
    try {
      ...
      const tasksMap = toRecord(tasksData, "id");
      set((state) => ({
        tasks: {
          ...state.tasks,
          ...tasksMap,
