...
import { APIDelete, APIPatch, APIPost, TaskCache, CreateTaskPayload, UpdateTaskPayload } from "@/types";
    ...
    async createTasks(
      ...
      return this.client
        ...
        .catch((error) => {
          throw (
            error?.response?.data ||
            new Error("Failed to create tasks")
          );
        });
    }
    async updateTasks(
      ...
      return this.client
        ...
        .catch((error) => {
          throw (
            error?.response?.data ||
            new Error("Failed to update tasks")
          );
        });
    }
    async deleteTasks(
