...
class ColumnService {
  ...
  async createColumns(
    ...
    return this.client
      ...
      .catch((error) => {
        throw error?.response?.data || new Error("Failed to create columns");
      });
  }
  async updateColumns(
    colIds: string[],
    ...
    return this.client
      ...
      .catch((error) => {
        throw error?.response?.data || new Error("Failed to update columns");
      });
  }
  async deleteColumns(colIds: string[]): Promise<APIDelete<boolean>> {
    return this.client
      .delete(`/column/${colIds.join(",")}`)
      .then((response) => response.data as APIDelete<boolean>)
      .catch((error) => {
        throw error?.response?.data || new Error("Failed to delete columns");
      });
  }
}
