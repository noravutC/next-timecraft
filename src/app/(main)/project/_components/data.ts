...
export type PendingMove =
  | { type: 'card'; taskId: string; fromColumnId: string; toColumnId: string; newOrderFraction: string }
...
export function deriveBoardView(
  ...
  for (const task of Object.values(tasksMap)) {
    ...
    if (pendingMove?.type === 'card' && task.id === pendingMove.taskId) continue;
...
export type TCardData = { [cardKey]: true; card: TCard; columnId: string; rect: DOMRect };

export function getCardData({ card, rect, columnId }: Omit<TCardData, typeof cardKey>): TCardData {
...
export function isDraggingACard({ source }: { source: { data: UnknownRecord } }): boolean {
  return isCardData(source.data);
}

const cardDropTargetKey = Symbol('card-drop-target');
export type TCardDropTargetData = { [cardDropTargetKey]: true; card: TCard; columnId: string };

export function getCardDropTargetData({ card, columnId }: Omit<TCardDropTargetData, typeof cardDropTargetKey>): TCardDropTargetData {
  return { [cardDropTargetKey]: true, card, columnId };
}
export function isCardDropTargetData(value: UnknownRecord): value is TCardDropTargetData {
...
export function isDraggingAColumn({ source }: { source: { data: UnknownRecord } }): boolean {
