...
import { Check, GalleryVerticalEnd, Plus, Search, Settings } from "lucide-react";
...
export const ProjectSwitch = ({ selectedProject }: ProjectSwitchProps) => {
    const { projects, projectIsUsing, setProjectIsUsing, setNeedCreateProject } = useProjectStore();
        ...
        <>
            <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
                <DropdownMenuTrigger asChild>
                    <div className={cn("flex items-center justify-start gap-4 min-w-60 p-2 rounded-md hover:bg-gray-100 cursor-pointer duration-300")}>
                        {/* <GalleryVerticalEnd /> */}
                        <Avatar className="size-10 rounded-sm">
                            ...
                            />
                            <AvatarFallback className={cn("rounded-sm bg-blue-100 text-blue-500 font-semibold")}>{selectedProject.name.charAt(0).toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                            <div className="text-gray-700 font-medium">{selectedProject.name}</div>
                            <div className="text-gray-500 !text-xs">{formatDateToString(selectedProject.createdAt)}</div>
                        </div>
                    </div>
                </DropdownMenuTrigger>

                <DropdownMenuContent className="w-90 flex flex-col p-0 pt-1 gap-1 mt-2" side="bottom" align="start">
                    ...
                    <div className="max-h-60 overflow-y-auto scrollbar-thin-y">
                        ...
                        {filteredProjects.map((project) => (
                            ...
                            onClick={() => setProjectIsUsing(project.id)}
                            >
                                <div className={cn("absolute left-0 w-[3px] rounded-tr rounded-br h-1/2 bg-blue-500",
                                    projectIsUsing === project.id ? 'opacity-100' : 'opacity-0'
                                )}></div>
                                <div className="flex items-center gap-3">
                                    {/* Project Avatar */}
                                    <Avatar className="border min-h-8 min-w-8 size-8 rounded-md">
                                        <AvatarImage src={project.coverImage ?? undefined} alt={project.name} />
                                ...
                                <Check className={cn("size-4 text-blue-500", projectIsUsing === project.id ? "opacity-100" : "opacity-0")} />
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="w-full mb-4 px-2">
                            ...
                            >
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs text-gray-500">Create a new workspace</span>
